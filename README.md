...
The whole project runs on one repeatable loop. It's **AI-heavy, workflow-oriented, and obsessed with documentation and traceability**. Every unit of work can be traced from idea to merged code.
...
7. **Simple tickets Claude Code remote fixes.** For simple fixes, that did not need human verification, I created a ticket and directly assigned Claude Code to fix it in the comments, without any kind of local coding, prompting or supervision.
...
| **Fresh context per iteration (Ralph loop)** | Each execute-plan iteration spawns a *new* subagent with a clean context window. This prevents accumulated bias and context rot: every pass re-reads the actual repo state and the plan, instead of trusting a stale internal narrative. |
...
| **Guardrails as code, not vibes** | Safety and quality are enforced by deterministic hooks and CI gates (below), not by hoping the model behaves. The AI literally *cannot* read a `.env`, write a hardcoded secret, or run `rm -rf`. |
...
| **100% test coverage gate, both stacks** | CI fails under 100% line coverage (backend `pytest`) and 100% across lines, branches, functions, and statements (frontend `vitest`). Combined with **randomized factory data** (`factory_boy`/`faker`), tests catch bugs that fixed fixtures would mask. |
...
| **Design tokens, never magic values** | All Figma colors, fonts, and spacing are centralized as Tailwind tokens; components consume `bg-cream`, `text-heading`, etc. `CLAUDE.md` rules prohibit hardcoded visual values, and ESLint + Prettier hooks keep the frontend clean on every write. |
...
### Figma MCP
Lets Claude read the design directly so interfaces are **pixel-perfect**. Before implementing any designed UI, Claude calls `get_design_context` and pulls exact colors, typography, spacing, and radii, never guessing or hardcoding visual values. (Worth noting: the Figma MCP needs *edit* access, so the canonical file is a **writable duplicate**, `imfHHa4B6WNdMXqhJh0pat`; the original challenge file is read-only and MCP-inaccessible.)
...
| [`dangerous-bash.sh`](.claude/hooks/dangerous-bash.sh) | Destructive shell commands like `rm -rf`, `git push --force`, `git reset --hard`, `DROP TABLE`, `--no-verify`. The agent simply cannot run them. |
...
- [`claude-code-review.yml`](.github/workflows/claude-code-review.yml) runs an automated Claude code review on every opened/updated PR.
