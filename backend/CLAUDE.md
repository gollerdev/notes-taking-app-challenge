...
│   │   ├── urls.py
│   │   └── tests/
│   │       └── factories.py         # UserFactory (shared across apps)
│   │
│   └── notes/
...
class NoteService:
    ...
    @staticmethod
    def partial_update(note, validated_data):
...
class NoteRepository:
    ...
    @staticmethod
    def update(note, validated_data):
...
**`.pre-commit-config.yaml`** (in `backend/`):
```yaml
repos:
...
Every model must have a companion factory. Factories live in `tests/factories.py` within their app. `UserFactory` lives in `authentication/tests/factories.py` and is imported wherever needed.
